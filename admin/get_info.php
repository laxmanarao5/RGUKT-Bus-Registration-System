         ...
         <br>
          <select class="opt" name="occ" style="width:300px;height:30px;">
            ...
            $row=$result->fetch_assoc();
                ...
                ?>
              <h1>Buses Required for <?php echo $occ;?><h1>
            <table class="table">
            ...
            <th scope="col">Buses Required</th>
            </tr>
        </thead>
        <tbody>
          <?php
            $count=0;
            $query1="SELECT DISTINCT location FROM `bookings`;";
		        $result1=mysqli_query($con,$query1);
            while($temp=$result1->fetch_assoc())
            {
              $loc=$temp['location'];
              $query="SELECT COUNT(location) as loc FROM bookings WHERE location='$loc'; ";
            $result=mysqli_query($con,$query);
            $row=$result->fetch_assoc();
                $count++;
              ?>
              <tr>
                ...
                <td><?php echo round($row['loc']/45);?></td>
              </tr>
            <?php
            }
          ?>
